...
type ProductFormTab =
  "details" | "platforms" | "variants" | "inventory" | "documents";
...
export function ProductFormPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([found, stockAll]) => {
        ...
        const forProduct = stockAll.filter(
          (s) => !s.deleted && s.productId === productId,
        );
  ...
  const variantStockMap = useMemo(
    () =>
      new Map(
        stockList.filter((s) => s.variantId).map((s) => [s.variantId!, s]),
      ),
    [stockList],
  );
  ...
  const changedVariants = useMemo(() => {
    ...
    return form.variants.filter((v) => {
      const target = Math.max(
        0,
        Math.floor(Number(variantQty[v.id] ?? "0") || 0),
      );
  ...
  const stockChanged =
    isEditing && (hasVariants ? changedVariants.length > 0 : stockDelta !== 0);
  ...
  const validate = (): boolean => {
    ...
    for (const listing of form.platformListings) {
      ...
      if (
        !isConfiguredMarketplace(listing.platform, marketplacesForValidation)
      ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && product) {
        await firestoreService.products.update(
          company.id,
          product.id,
          {
            name: form.name.trim(),
        ...
        if (stockChanged) {
          try {
            ...
            notification.error(
              "Product saved, but the stock adjustment failed.",
            );
            setSaving(false);
            return;
          }
      ...
      if (code.includes("storage")) {
        notification.error(
          "Failed to upload document. Use JPEG, PNG, GIF, WebP, HEIC, or PDF (max 10 MB).",
        );
      } else {
          ...
          <FormPanel role="tabpanel">
            ...
            {activeTab === "platforms" ? (
              <PlatformListingEditor
                ...
                onChange={(platformListings) =>
                  setForm((f) => ({ ...f, platformListings }))
                }
                company={company}
                currency={currency}
              ...
              <div className="max-w-3xl space-y-4">
                {hasVariants ? (
                  <>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Set the on-hand count per variant. Purchases and sales
                      still update stock automatically — every manual change is
                      logged.
                    </p>
                      ...
                      <table className={tableClass}>
                        <thead>
                          <tr className={tableHeadRowClass}>
                            ...
                            <th className={`${tableHeadCellClass} w-28`}>
                              Current
                            </th>
                            <th className={`${tableHeadCellClass} w-40`}>
                              New on-hand
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {form.variants.map((variant) => {
                            ...
                            const target = Math.max(
                              0,
                              Math.floor(
                                Number(variantQty[variant.id] ?? "0") || 0,
                              ),
                            );
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Manually correct the on-hand count (e.g. after a stock
                      take). Purchases and sales still update stock
