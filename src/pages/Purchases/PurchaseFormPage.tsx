...
export function PurchaseFormPage() {
  ...
  const preview = useMemo(
    () => computePurchasePreview(form, products),
    [form, products],
  );
    ...
    const load = async () => {
      try {
        ...
        if (isEditing && purchaseId) {
          const found = await firestoreService.purchases.get(
            company.id,
            purchaseId,
          );
          if (cancelled) return;
  ...
  const updateLine = (
    lineId: string,
    patch: Partial<PurchaseFormState["lines"][0]>,
  ) => {
    setForm((prev) => ({
      ...prev,
  ...
  const addLine = () => {
    setForm((prev) => ({
      ...prev,
      lines: [...prev.lines, emptyPurchaseLineForm()],
    }));
    setActiveTab("items");
  };
  ...
  const fillFromProduct = (lineId: string, productId: string) => {
    ...
    updateLine(lineId, {
      ...
      taxPercentage: String(
        listing?.purchaseTaxPercentage ?? listing?.taxPercentage ?? 0,
      ),
      taxMode:
        listing?.purchaseTaxMode ?? listing?.taxMode ?? TaxMode.INCLUSIVE,
    });
  };
  ...
  const fillFromVariant = (lineId: string, variantId: string) => {
    ...
    if (variant.purchasePrice != null)
      patch.purchasePrice = String(variant.purchasePrice);
    if (variant.sellingPrice != null)
      patch.sellingPrice = String(variant.sellingPrice);
  ...
  const validate = (): boolean => {
    ...
    if (!next.lines) {
      ...
      if (missingVariant)
        next.lines = "Select a variant for each product that has variants";
    }
    setErrors(next);
    ...
    try {
      ...
      if (isEditing && purchase) {
        ...
        if (cancelling || !isPurchaseReceiptLocked(purchase)) {
          try {
            await syncPurchaseStockReceipts(
              company.id,
              purchase,
              payload,
              user!.uid,
            ...
            notification.error(
              stockErr instanceof Error
                ? stockErr.message
                : "Could not update stock receipts",
            );
            setSaving(false);
            return;
          }
        ...
        const created = await firestoreService.purchases.create(
          company.id,
          payload,
          user!.uid,
        );
  ...
  const cancelTo =
    isEditing && purchase ? `/purchases/${purchase.id}` : "/purchases";
    ...
    <PageShell>
      ...
      {activeProducts.length === 0 ? (
        ...
        <FormPageBody id="purchase-form" onSubmit={handleSubmit}>
                ...
                {form.vendorId ? (
                  <p className="text-xs text-gray-500 dark:text-gray-400 pt-1 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <span className="font-medium text-gray-700 dark:text-gray-300">
                      {activeVendors.find((v) => v.id === form.vendorId)
                        ?.name ?? "—"}
                    </span>
                  </p>
                ) : null}
              ...
              {activeTab === "order" ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <Input
                    label="PO number"
                    value={
                      isEditing ? (purchase?.poNumber ?? "") : nextPoNumber
                    }
                    readOnly
                    disabled
                    helperText={
                        ...
                        {form.lines.map((line, i) => (
                          <PurchaseLineEditor
                            ...
                            onProductSelect={(productId) =>
                              fillFromProduct(line.id, productId)
                            }
                            onVariantSelect={(variantId) =>
                              fillFromVariant(line.id, variantId)
                            }
          ...
          {isReady ? (
            <FormReadyBanner>
              {filledLines} line{filledLines === 1 ? "" : "s"} — ready to
              create.
            </FormReadyBanner>
          ) : null}
