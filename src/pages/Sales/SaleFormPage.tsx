...
type SaleFormTab =
  "order" | "customer" | "items" | "delivery" | "extras" | "documents";
...
export function SaleFormPage() {
  ...
  const formProducts = useMemo(() => {
    ...
    const extras = products.filter(
      (p) => saleProductIds.has(p.id) && !active.some((a) => a.id === p.id),
    );
  ...
  const tracksTax =
    (firstLineEconomics?.taxType ?? TaxType.NONE) !== TaxType.NONE;
  ...
  const updateLine = (
    lineId: string,
    patch: Partial<(typeof form.lines)[0]>,
  ) => {
    setForm((f) => ({
      ...f,
  ...
  const applyStatusChange = (status: SaleStatus) => {
    setForm((f) => {
      ...
      if (status === SaleStatus.RETURNED) {
        ...
        if (
          f.returnTaxPercentage === 0 &&
          (econ?.deliveryTaxPercentage ?? 0) > 0
        ) {
  ...
  const requestStatusChange = (status: SaleStatus) => {
    ...
    if (status === SaleStatus.RETURNED) {
      ...
      message +=
        " You can enter cancellation charges and tax after confirming.";
    } else if (
    ...
    try {
      ...
      if (isEditing && sale) {
        ...
        try {
          ...
          notification.error(
            "Sale saved but linked expenses could not be updated.",
          );
        }
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description={
          isEditing
            ? `Order ${sale?.orderNumber ?? sale?.orderId ?? ""}`
            : "Marketplace order"
        }
        actions={
          !noProducts ? (
                ...
                <div className="space-y-3">
                  ...
                  {!form.platform ? (
                    <p className="text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md px-3 py-2">
                      Go to the Order tab and select a marketplace before adding
                      products.
                    </p>
                  ) : (
                    <>
                        ...
                        <table className={tableClass}>
                          <thead>
                            <tr className={tableHeadRowClass}>
                              ...
                              <th className={`${tableHeadCellClass} w-16`}>
                                Qty
                              </th>
                              <th className={`${tableHeadCellClass} w-24`}>
                                Cost
                              </th>
                              <th className={`${tableHeadCellClass} w-24`}>
                                Price
                              ...
                              <th
                                className={`${tableHeadCellClass} w-24 text-right`}
                              >
                                Profit
                              </th>
                            ...
                            {form.lines.map((line, index) => (
                              <SaleLineEditor
                                ...
                                onEconomicsChange={(patch) =>
                                  updateLineEconomics(line.id, patch)
                                }
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        requestStatusChange(e.target.value as SaleStatus)
                      }
                    />
                    {isReturned ? (
                      <Input
                        ...
                        onChange={(e) =>
                          setForm((f) => ({ ...f, returnedAt: e.target.value }))
                        }
                      />
                    ) : null}
                    {isCancelled ? (
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({ ...f, trackingId: e.target.value }))
                      }
                  ...
                  {isReturned ? (
                    <OutcomeChargeFields
                      ...
                      onTaxModeChange={(mode) =>
                        setForm((f) => ({ ...f, returnTaxMode: mode }))
                      }
                      tracksTax={tracksTax}
                      pctLabel={pctLabel}
                      currency={currency}
                  ...
                  {isCancelled ? (
                    <OutcomeChargeFields
                      ...
                      onTaxPercentageChange={(value) =>
                        setForm((f) => ({
                          ...f,
                          cancellationTaxPercentage: value,
                        }))
                      }
          ...
          {isReady ? (
            <FormReadyBanner>
              ...
              <span className="font-medium">
                {orderNumberPreview || "ORD-…"}
              </span>{" "}
              on <span className="font-medium">{form.platform}</span> — ready to
              log.
            </FormReadyBanner>
          ) : null}
