...
async function getLinkedSaleExpenses(companyId: string, saleId: string): Promise<Expense[]> {
  const expenses = await firestoreService.expenses.getBySaleId(companyId, saleId);
  return expenses.filter((e) => e.autoGenerated && !e.deleted);
}

/** Create or update linked expenses for a sale; remove stale auto-generated lines. */
export async function syncSaleExpenses(
  companyId: string,
  sale: Sale,
  deletedBy: string
): Promise<void> {
  const linked = await getLinkedSaleExpenses(companyId, sale.id);
  const linkedByKind = new Map(linked.map((e) => [e.saleExpenseKind, e]));

  const pendingCreates: Array<{ expense: Expense }> = [];
  const pendingUpdates: Array<{ id: string; expense: Expense }> = [];
  const pendingDeletes: string[] = [];

  for (const kind of ALL_SALE_EXPENSE_KINDS) {
    const existing = linkedByKind.get(kind);
    const spec = specForKind(sale, kind);

    if (!spec) {
      if (existing) pendingDeletes.push(existing.id);
      continue;
    }
    ...
    if (existing) {
      pendingUpdates.push({ id: existing.id, expense });
    } else {
      pendingCreates.push({ expense });
    }
  }

  await Promise.all([
    ...pendingDeletes.map((id) => firestoreService.expenses.delete(companyId, id, deletedBy)),
    ...pendingUpdates.map(({ id, expense }) =>
      firestoreService.expenses.update(companyId, id, expense, deletedBy)
    ),
    ...pendingCreates.map(({ expense }) =>
      firestoreService.expenses.create(companyId, expense, deletedBy)
    ),
  ]);
}
...
export async function deleteSaleLinkedExpenses(
  ...
  const linked = await getLinkedSaleExpenses(companyId, saleId);
