...
import { InvoiceStatus, PaymentKind, PurchaseOrderStatus, PurchasePaymentStatus, SaleExpenseKind, SaleStatus } from '../types';
import {
  localDateInputToUtc,
...
/** Sales that count toward revenue and profit (not deleted, cancelled or returned). */
export function isReportableSale(sale: Sale): boolean {
  return (
    !sale.deleted &&
...
export function filterSalesInRange(sales: Sale[], from?: Date, to?: Date): Sale[] {
  return sales
    ...
    .filter((s) => isDateInRange(s.orderDate, from, to));
}
...
export function computeSaleReceivables(sales: Sale[]): InvoiceReceivablesSummary {
  ...
  for (const sale of sales) {
    if (sale.deleted || sale.status === SaleStatus.CANCELLED) continue;
    const total = sale.total ?? sale.grossRevenue;
    totalInvoiced += total;
    // Mirror the customer-ledger fallback so Dashboard AR matches the customer
    // pages even when balanceDue was never persisted on older records.
    const due = sale.balanceDue ?? Math.max(0, total - (sale.totalPaid ?? 0));
    if (due > 0) {
      balanceDue += due;
      openCount++;
    }
...
export function isExpenseExcludedFromNetProfit(expense: Expense): boolean {
  ...
  if (expense.saleId) {
    // Return/cancellation fees are genuine costs. The underlying sale is
    // excluded from profit aggregation (see isReportableSale), so these must
    // still reduce net profit instead of being netted out with the order.
    if (
      expense.saleExpenseKind === SaleExpenseKind.RETURN_CHARGES ||
      expense.saleExpenseKind === SaleExpenseKind.CANCELLATION_CHARGES
    ) {
      return false;
    }
    return true;
